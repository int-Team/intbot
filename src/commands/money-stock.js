...
module.exports = {
  ...
  run: async (client, message, args, ops) => {
    ...
    let embed = new Discord.MessageEmbed()
      .setTitle("주식 현황")
      .setDescription(
        `다음 변동까지 ${timeToKorean(
          float2int(
            600 -
              Math.abs((10000 - (Date.now() - client.lastStockUpdate)) / 1000)
          )
        )}`
      )
      .setColor("GREEN")
    ...
    for (let stock of stocks) {
      ...
      if (previousStockPM == "0") previousStockPM = "---";
      else if (!isNaN(previousStockPM)) previousStockPM = "+";
      if (previousStockPM == "+") previousStockPM2 = "▲";
      else if (previousStockPM == "-") previousStockPM2 = "▼";
      else previousStockPM2 = "~";

      previousStockPM += " ";
      let previousStockStr = `${previousStockPM}(${previousStockPM2} ${numberWithCommas(
        previousStock
      )})`;
    ...
    embed.addField(
      ...
      }시 ${lastUpdate.getMinutes()}분 ${lastUpdate.getSeconds()}초`,
      true
    );
