...
module.exports = {
  ...
  run: async (client, message, args, ops) => {
    ...
    if (!(await client.db.findOne({ _id: message.author.id }))) {
      ...
      axios
        .get(`https://api.koreanbots.dev/bots/voted/${message.author.id}`, {
          headers: {
        ...
        .catch((e) => {
          message.channel.send(
            `저런... 개발중에 뭘 잘못 건든거 같네요. 개발자들이 이 오류를 보면 얼마나 스트래쓰를 받을지는 모르겠지만 일단 보낼께요. \n${e}`
          )
        })
      if (
        (
          await axios.get(
            `https://api.koreanbots.dev/bots/voted/${message.author.id}`,
            {
              headers: {
                token: koreanbottoken,
              },
        ...
        if (
          (await client.db.findOne({ _id: message.author.id })).lastGotMoney &&
          new Date() -
            1 +
            1 -
            (await client.db.findOne({ _id: message.author.id })).lastGotMoney <
          ...
          await client.db.updateOne(
            { _id: message.author.id },
            {
              $set: {
                ...
                money:
                  (await client.db.findOne({ _id: message.author.id })).money +
                  1000,
              },
            }
          )
            ...
            .setDescription(
              `${
                ((7200000 -
                  ...
                  1000) |
                0
              }초 후에 다시 시도해 주세요!\nKOREANBOTS에서 [여기](https://koreanbots.dev/bots/${
                client.user.id
              })를 눌러 하트를 추가하면 클타임이 5분이에요!`
            )
            .setColor('RANDOM')
            .setFooter(message.author.tag, message.author.displayAvatarURL())
            .setTimestamp()
